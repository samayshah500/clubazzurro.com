...
update_option("envato_purchase_code_19264426", "activated");
...
function grandtour_add_admin() 
{
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'functions.php' ) {
		...
		if ( isset($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
			...
			{
				$css_cache_path = GRANDTOUR_THEMEUPLOAD.'/custom-css.css';
				...
				if(!file_exists($css_cache_path))
				{
					...
					$grandtour_custom_css = ob_get_contents(); 
					ob_end_clean();
					
					//Writing cache file
					file_put_contents($css_cache_path, $grandtour_custom_css);
				}
			}
...
function grandtour_enqueue_front_page_scripts() 
{
	...
	else
	{
		...
		{
			$css_cache_path = GRANDTOUR_THEMEUPLOAD.'/custom-css.css';
			$cache_url = GRANDTOUR_THEMEUPLOADURL.'custom-css.css';
			...
			wp_enqueue_style("grandtour-custom-css", $cache_url, false, "", "all");
		}
		else
		{
