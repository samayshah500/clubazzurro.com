...
Plugin URI: https://themes.themegoods2.com/grandtour/demo/
...
if($is_verified_envato_purchase_code) {
	...
	function post_type_galleries() {
		...
		$tg_permalink_galleries = get_option(GRANDTOUR_SHORTNAME.'_permalink_galleries');
		if(empty($tg_permalink_galleries)) {
			$tg_permalink_galleries = 'galleries';
		}
		
		$tg_permalink_gallerycat = get_option(GRANDTOUR_SHORTNAME.'_permalink_gallerycat');
	...
	function post_type_tours() {
		//Get custom permalink setting
		$tg_permalink_tour = get_option(GRANDTOUR_SHORTNAME.'_permalink_tour');
		if(empty($tg_permalink_tour)) {
			$tg_permalink_tour = 'tour';
		}
		
		$tg_permalink_tourcat = get_option(GRANDTOUR_SHORTNAME.'_permalink_tourcat');
		if(empty($tg_permalink_tourcat)) {
			$tg_permalink_tourcat = 'tourcat';
		}
		...
		$args = array(
    		...
    		'show_ui' => true, 
    		'query_var' => true,
    		'rewrite' => array('slug' => $tg_permalink_tour,'with_front' => false),
    		'capability_type' => 'post',
    		'hierarchical' => false,
    		'menu_position' => null,
		...
		register_post_type( 'tour', $args );
  		...
  		register_taxonomy(
			...
			array(
				...
				'query_var' => 'tourcat',
				'show_ui' => true,
				'rewrite' => array( 'slug' => $tg_permalink_tourcat, 'with_front' => false ),
			)
		);		  
	}
	...
	function post_type_destination() {
		//Get custom permalink setting
		$tg_permalink_destination = get_option(GRANDTOUR_SHORTNAME.'_permalink_destination');
		if(empty($tg_permalink_destination)) {
			$tg_permalink_destination = 'destination';
		}
		
		$tg_permalink_destinationcat = get_option(GRANDTOUR_SHORTNAME.'_permalink_destinationcat');
		if(empty($tg_permalink_destinationcat)) {
			$tg_permalink_destinationcat = 'destinationcat';
		}
		...
		$args = array(
    		...
    		'show_ui' => true, 
    		'query_var' => true,
    		'rewrite' => array('slug' => $tg_permalink_destination,'with_front' => false),
    		'capability_type' => 'post',
    		'hierarchical' => false,
    		'menu_position' => null,
		...
		register_post_type( 'destination', $args );
  		...
  		register_taxonomy(
			...
			array(
				...
				'show_ui' => true,
				'rewrite' => array( 'slug' => $tg_permalink_destinationcat, 'with_front' => false ),
			)
		);	
	} 
